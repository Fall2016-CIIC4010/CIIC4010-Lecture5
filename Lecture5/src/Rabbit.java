...
public class Rabbit implements Raceable {
	...
	public Rabbit(int x, int y, Color color, int hSpeed, int HDir, int number) {
		...
		File theFile = new File("image/bugs-medium.jpg");

		try {
			this.image =  ImageIO.read(theFile);
		} catch (IOException e) {
			System.out.println("Fatal Error!!!!: Image file not found");
			System.exit(1);
		}
	}
